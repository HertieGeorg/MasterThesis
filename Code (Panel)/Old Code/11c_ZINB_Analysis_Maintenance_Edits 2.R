...
df_Zinb$All_Maintenance_CongressEdits_Per_MoC_Session <- df_Zinb$AllCongressEdits_Per_MoC_Session - df_Zinb$All_Politically_CongressEdits_Per_MoC_Session
...
100*sum(df_Zinb$All_Maintenance_CongressEdits_Per_MoC_Session == 0)/nrow(df_Zinb) #72.06% sind Zeros

#Let’s start with the simplest model, a Poisson GLM: 

M1 <- glm(All_Maintenance_CongressEdits_Per_MoC_Session ~  vote_maxdiff_relative  + sex + birthyear +
...
M1_quasi <- glm(All_Maintenance_CongressEdits_Per_MoC_Session ~  vote_maxdiff_relative  + sex + birthyear + 
                  ExternalEdits_per_MoC_Session + session + party_dual + Chamber,
          family=quasipoisson,
          data = df_Zinb)
...
M2_negBinom <- glm.nb(All_Maintenance_CongressEdits_Per_MoC_Session ~  vote_maxdiff_relative  + sex + birthyear +
...
M3_ZeroInfl <- zeroinfl(All_Maintenance_CongressEdits_Per_MoC_Session ~  vote_maxdiff_relative   + sex + birthyear + ExternalEdits_per_MoC_Session + session + party_dual + Chamber  | session + ExternalEdits_per_MoC_Session ,
                        data = df_Zinb , dist = "negbin")

M3_ZeroInfl2 <- zeroinfl(All_Maintenance_CongressEdits_Per_MoC_Session ~  vote_maxdiff_relative  + sex + birthyear +
...
stargazer(M1, M1_quasi, M2_negBinom, M3_ZeroInfl, 
          ...
          out="MaintenanceEdits(1)_Model_Results.html",  type = "text", 
...
quantile(df_Zinb$All_Maintenance_CongressEdits_Per_MoC_Session, prob = seq(0, 1, length = 200), type = 5) #Top 1% = bigger than 10
df_Zinb_AltSpecA <- df_Zinb %>% filter(All_Maintenance_CongressEdits_Per_MoC_Session <= 5)

M3_ZeroInfl_AltSpecA <- zeroinfl(All_Maintenance_CongressEdits_Per_MoC_Session ~  vote_maxdiff_relative  + sex + birthyear +
...
M3_ZeroInfl_View <- zeroinfl(All_Maintenance_CongressEdits_Per_MoC_Session ~  vote_maxdiff_relative  + sex + birthyear + ExternalEdits_per_MoC_Session + session + party_dual + Chamber + ViewCategory  | session + ViewCategory + ExternalEdits_per_MoC_Session,
                               data = df_Zinb , dist = "negbin")
...
M3_ZeroInfl_AltSpec_NoDoubles <- zeroinfl(All_Maintenance_CongressEdits_Per_MoC_Session ~  vote_maxdiff_relative + sex + birthyear +
                                   ExternalEdits_per_MoC_Session + session + party_dual + Chamber | session + ExternalEdits_per_MoC_Session ,
...
# Out: MaintenanceEdits(2)_ZinB_Specifications.html
...
stargazer(M3_ZeroInfl, M3_ZeroInfl_AltSpecA, M3_ZeroInfl_AltSpec_NoDoubles, M3_ZeroInfl_View,
          ...
          out="MaintenanceEdits(2)_ZinB_Specifications.html",  type = "text", 
...
# Ad 1/2
M3_ZeroInfl_AltSpec2 <- zeroinfl(All_Maintenance_CongressEdits_Per_MoC_Session ~ Category_Vote_MaxDiff_2 + sex + birthyear +
                                   ExternalEdits_per_MoC_Session + session + party_dual + Chamber | session + ExternalEdits_per_MoC_Session ,
                                 data = df_Zinb , dist = "negbin")
...
# Ad 1/3
M3_ZeroInfl_AltSpec3 <- zeroinfl(All_Maintenance_CongressEdits_Per_MoC_Session ~ Category_Vote_MaxDiff_3 + sex + birthyear +
                                   ExternalEdits_per_MoC_Session + session + party_dual + Chamber | session + ExternalEdits_per_MoC_Session,
                                 data = df_Zinb , dist = "negbin")
...
# Ad 1/4
M3_ZeroInfl_AltSpec4 <- zeroinfl(All_Maintenance_CongressEdits_Per_MoC_Session ~ Category_Vote_MaxDiff_4 + sex + birthyear +
                                   ExternalEdits_per_MoC_Session + session + party_dual + Chamber | session + ExternalEdits_per_MoC_Session,
                                 data = df_Zinb , dist = "negbin")
...
# Ad 1/5
M3_ZeroInfl_AltSpec5 <- zeroinfl(All_Maintenance_CongressEdits_Per_MoC_Session ~ Category_Vote_MaxDiff_5 + sex + birthyear +
                                   ExternalEdits_per_MoC_Session + session + party_dual + Chamber | session + ExternalEdits_per_MoC_Session ,
                                 data = df_Zinb , dist = "negbin")
...
# Ad 1/6
M3_ZeroInfl_AltSpec6 <- zeroinfl(All_Maintenance_CongressEdits_Per_MoC_Session ~ Category_Vote_MaxDiff_6 + sex + birthyear +
                                   ExternalEdits_per_MoC_Session + session + party_dual + Chamber | session + ExternalEdits_per_MoC_Session ,
                                 data = df_Zinb , dist = "negbin")
...
# Out: MaintenanceEdits(3_Appendix)_ZinB_VoteDiffCategories.html 
...
stargazer(M3_ZeroInfl_AltSpec2, M3_ZeroInfl_AltSpec3, M3_ZeroInfl_AltSpec4, M3_ZeroInfl_AltSpec5, M3_ZeroInfl_AltSpec6, M3_ZeroInfl,
          ...
          out="MaintenanceEdits(3_Appendix)_ZinB_VoteDiffCategories.html",  type = "text",  
...
M3_ZeroInfl_AltCount <- zeroinfl(All_Maintenance_CongressEdits_Per_MoC_Session ~ vote_maxdiff_relative + sex + birthyear +
...
M3_ZeroInfl_AltCount2 <- zeroinfl(All_Maintenance_CongressEdits_Per_MoC_Session ~ vote_maxdiff_relative + sex + birthyear +
...
# Out: MaintenanceEdits(4_Appendix)_ZinB_DifferentCounts.html
stargazer(M3_ZeroInfl, M3_ZeroInfl_AltCount, M3_ZeroInfl_AltCount2, 
          ...
          out="MaintenanceEdits(4_Appendix)_ZinB_DifferentCounts.html",  type = "text", 
          title="Zero-inflated models: different variables in logit part", align=TRUE) 



# Out: MaintenanceEdits(5_Appendix)_ZinB_DifferentCounts.html
