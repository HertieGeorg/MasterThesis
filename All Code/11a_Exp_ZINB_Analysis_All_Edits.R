...
M3_ZIP <- zeroinfl(AllCongressEdits_Per_MoC_Session ~  vote_maxdiff_relative   + sex + birthyear + 
...
# Source for standard errors: https://data.library.virginia.edu/understanding-robust-standard-errors/
# Another source for code for standard errors: https://stackoverflow.com/questions/35372090/clustered-standard-error-for-zero-inflated-negative-binomial-model
# Another source for code for standard erros in stargazer: https://cran.r-project.org/web/packages/stargazer/vignettes/stargazer.pdf
...
M4_Hurdle_robust.se_Count <- M4_Hurdle_robust.se[1:12]
M4_Hurdle_robust.se_Zero <- M4_Hurdle_robust.se [13:24]
...
M3_ZIP_robust.se_Count <- M3_ZIP_robust.se[1:12]
M3_ZIP_robust.se_Zero <- M3_ZIP_robust.se[13:24]
...
# Out: AlllEdits(1_Appendix)_Model_Results_Count.html
stargazer(M1,  M2_negBinom, M4_Hurdle, M3_ZIP, M3_ZeroInfl, 
          se = list(M1_robust.se, M2_negBinom_robust.se,M4_Hurdle_robust.se_Count, M3_ZIP_robust.se_Count, M3_ZeroInfl_robust.se_Count), 
          ...
          title="Table A1. Modeling the number of Congress edits: Comparison of models", model.names = F, 
          ...
          out="Table A1 (AllEdits(Table_A1_Appendix)_Model_Results_Count).html",  type = "text")


# Out: AlllEdits(1_Appendix)_Model_Results_Zero.html
stargazer(M4_Hurdle, M3_ZIP, M3_ZeroInfl,  zero.component = TRUE,
          se = list(M4_Hurdle_robust.se_Zero , M3_ZIP_robust.se_Zero , M3_ZeroInfl_robust.se_Zero ), 
          ...
          covariate.labels=c("District Competitiveness", "Gender(Male)", "Age", "External Edits", 
                             "110th Session", "111th Session", "112th Session", "113th Session", 
                             "114th Session", "Party Affiliation(R)", "Chamber(Senate)"),
          title="Table A2. Modeling the likelihood of Congress edits: Comparison of models", model.names = F, 
          notes.append = F, notes.align = "r",
          notes = c("\\parbox[t]{\\textwidth}{Age is higher for younger legislators (based on year of birth). Independents are allocated to the party they caucus with. }", 
                    "\\parbox[t]{\\textwidth}{District competitiveness is computed as the relative difference in votes to the second best candidate.}",
                    "\\parbox[t]{\\textwidth}{109th session as baseline. Cluster-robust standard errors taken by legislator.}", 
                    ...
                    "\\parbox[t]{\\textwidth}{ *p<0.1; **p<0.05; ***p<0.01}"),
          align=TRUE, omit.stat=c("aic", "theta" ),
          out="Table A2 (AllEdits(Table_A2_Appendix)_Model_Results_Zero).html",  type = "text")
...
df    <-  c(12, 13, 25, 24, 25) 
AIC   <-  c(10376.358, 6989.523, 6954.552, 8065.959, 6944.784) 
BIC   <-  c(10449.643, 7068.916, 7107.230, 8212.530, 7097.462) 
...
M3_ZeroInfl_AltSpecE <- zeroinfl(AllCongressEdits_Per_MoC_Session ~  vote_maxdiff_relative  + sex + birthyear +
                                   ExternalEdits_per_MoC_Session + session + party_dual + Chamber | vote_maxdiff_relative  + sex + birthyear +
                                   ExternalEdits_per_MoC_Session + session + party_dual + Chamber,
...
source("12_(Table1&2)_Exponentiated_Coefficients.R")
...
stargazer(M3_ZeroInfl, M3_ZeroInfl_AltSpecA, M3_ZeroInfl_AltSpecE,  
          ...
          covariate.labels=c("District Competitiveness", "Gender(Male)", "Age", "External Edits", 
                             "110th Session", "111th Session", "112th Session", "113th Session", 
                             "114th Session", "Party Affiliation(R)", "Chamber(Senate)"),
          title="Table 2. Modelling the Number of Congressional Edits using ZINB: Different Specifications (non exponentiated results)", model.names = F, 
          notes.append = F, notes.align = "r",
          notes = c("\\parbox[t]{\\textwidth}{Age is higher for younger legislators (based on year of birth). Independents are allocated to the party they caucus with. }", 
                    "\\parbox[t]{\\textwidth}{109th session as baseline. Cluster-robust standard errors taken by legislator.}",  
                    "\\parbox[t]{\\textwidth}{Coef and SE are not exponeniated.}",
                    "\\parbox[t]{\\textwidth}{ *p<0.1; **p<0.05; ***p<0.01}"),
          align=TRUE, omit.stat=c("aic", "theta" ),
          #out="AllEdits(Table_2_NonExpo)_ZinB_Specifications_Count.html", 
          type = "text")
...
stargazer(M3_ZeroInfl, M3_ZeroInfl_AltSpecA, M3_ZeroInfl_AltSpecE,  zero.component = TRUE,
          ...
          covariate.labels=c("District Competitiveness", "Gender(Male)", "Age", "External Edits", 
                             "110th Session", "111th Session", "112th Session", "113th Session", 
                             "114th Session", "Party Affiliation(R)", "Chamber(Senate)"),
          title="Table 1. Modeling the likelihood of Congressional Edits using ZINB: Different Specifications (non exponentiated results)", model.names = F, 
          #style = "qje",  #model.numbers  = FALSE
          notes.append = F, notes.align = "r",
          notes = c("\\parbox[t]{\\textwidth}{Age is higher for younger legislators (based on year of birth). Independents are allocated to the party they caucus with. }", 
                    "\\parbox[t]{\\textwidth}{109th session as baseline. Cluster-robust standard errors taken by legislator.}",  
                    ...
                    "\\parbox[t]{\\textwidth}{ *p<0.1; **p<0.05; ***p<0.01}"),
          align=TRUE, omit.stat=c("aic", "theta" ),
          #out="AllEdits(Table_1_NonExpo)_ZinB_Specifications_Zero.html", 
          type = "text")
...
M3_ZeroInfl_AltCount3 <- zeroinfl(AllCongressEdits_Per_MoC_Session ~ vote_maxdiff_relative + sex + birthyear +
                                    ExternalEdits_per_MoC_Session + session + party_dual + Chamber | sex + birthyear + party_dual + Chamber,
                                  data = df_Zinb , dist = "negbin")
#M3_ZeroInfl_AltCount2_Robust <- coeftest(M3_ZeroInfl_AltCount2, vcov = vcovCL(M3_ZeroInfl_AltCount2, cluster = df_Zinb$pageid))
...
# Out: AllEdits(3_Appendix)_ZinB_DifferentZeros_Count.html - Count part: OUTPUT NOT INCLUDED INTO THESIS 
stargazer(M3_ZeroInfl, M3_ZeroInfl_AltCount2, M3_ZeroInfl_AltCount3, 
          ...
          covariate.labels=c("District Competitiveness", "Gender(Male)", "Age", "External Edits", 
                             "110th Session", "111th Session", "112th Session", "113th Session", 
                             "114th Session", "Party Affiliation(R)", "Chamber(Senate)"),
          title="Table X1. Modelling the Number of Congressional Edits using ZINB: Variations in the Inflation Part (logit) (non exponentiated results)", model.names = F, 
                   notes.append = F, notes.align = "r",
          notes = c("\\parbox[t]{\\textwidth}{Age is higher for younger legislators (based on year of birth). Independents are allocated to the party they caucus with. }", 
                    "\\parbox[t]{\\textwidth}{109th session as baseline. Cluster-robust standard errors taken by legislator.}",  
                    "\\parbox[t]{\\textwidth}{Coef and SE are not exponeniated.}",
                    "\\parbox[t]{\\textwidth}{ *p<0.1; **p<0.05; ***p<0.01}"),
          align=TRUE, omit.stat=c("aic", "theta" ),
          #out="AllEdits(Table_X1_Appendix)_ZinB_DifferentZeros_Count.html", 
          type = "text")



# Out: AllEdits(3_Appendix)_ZinB_DifferentZeros_Zero.html - Zero part: OUTPUT NOT INCLUDED INTO THESIS 
stargazer(M3_ZeroInfl, M3_ZeroInfl_AltCount2, M3_ZeroInfl_AltCount3, zero.component = TRUE,
          ...
          covariate.labels=c("District Competitiveness", "Gender(Male)", "Age", "External Edits", 
                             "110th Session", "111th Session", "112th Session", "113th Session", 
                             "114th Session", "Party Affiliation(R)", "Chamber(Senate)"),
          title="Table X1. Modeling the likelihood of Congressional Edits using ZINB: Variations in the Inflation Part (logit) (non exponentiated results)", model.names = F, 
          #style = "qje",  #model.numbers  = FALSE
          notes.append = F, notes.align = "r",
          notes = c("\\parbox[t]{\\textwidth}{Age is higher for younger legislators (based on year of birth). Independents are allocated to the party they caucus with. }", 
                    "\\parbox[t]{\\textwidth}{109th session as baseline. Cluster-robust standard errors taken by legislator.}",  
                    ...
                    "\\parbox[t]{\\textwidth}{ *p<0.1; **p<0.05; ***p<0.01}"),
          align=TRUE, omit.stat=c("aic", "theta" ),
          #out="AllEdits(Table_X2_Appendix)_ZinB_DifferentZeros_Zero.html",  
          type = "text")
...
stargazer(M3_ZeroInfl, M3_ZeroInfl_AltSpecA, M3_ZeroInfl_AltSpecE, 
          ...
          dep.var.labels=c(""), 
          column.labels = c("Full Sample", "99 Percentile of<br>Congress Edits", "99 Percentile of<br>External Edits",  
                            "Non person-specific<br>characteristics", "Person-specific<br>characteristics"),
          covariate.labels=c("District Competitiveness", "Gender(Male)", "Age", "External Edits", 
                             "110th Session", "111th Session", "112th Session", "113th Session", 
                             "114th Session", "Party Affiliation(R)", "Chamber(Senate)"),
          title="Table A5. Modeling the number of Congress edits using ZINB: Different specifications", model.names = F, 
          notes.append = F, notes.align = "r",
          notes = c("\\parbox[t]{\\textwidth}{Age is higher for younger legislators (based on year of birth). Independents are allocated to the party they caucus with. }", 
                    "\\parbox[t]{\\textwidth}{District competitiveness is computed as the relative difference in votes to the second best candidate.}",
                     "\\parbox[t]{\\textwidth}{109th session as baseline. Cluster-robust standard errors taken by legislator.}",  
                    ...
                    "\\parbox[t]{\\textwidth}{ *p<0.1; **p<0.05; ***p<0.01}"),
          align=TRUE, omit.stat=c("aic", "theta" ),
          out="Table A5 (llEdits(Table_A3_Appendix)_ZinB_Specifications_Count).html", 
          type = "text")
...
stargazer(M3_ZeroInfl, M3_ZeroInfl_AltSpecA, M3_ZeroInfl_AltSpecE,
          ...
          covariate.labels=c("District Competitiveness", "Gender(Male)", "Age", "External Edits", 
                             "110th Session", "111th Session", "112th Session", "113th Session", 
                             "114th Session", "Party Affiliation(R)", "Chamber(Senate)"),
          title="Table A4. Modeling the likelihood of Congress edits using ZINB: Different specifications", model.names = F, 
          #style = "qje",  #model.numbers  = FALSE
          notes.append = F, notes.align = "r",
          notes = c("\\parbox[t]{\\textwidth}{Age is higher for younger legislators (based on year of birth). Independents are allocated to the party they caucus with. }", 
                    "\\parbox[t]{\\textwidth}{District competitiveness is computed as the relative difference in votes to the second best candidate.}",
                    ...
                    "\\parbox[t]{\\textwidth}{ *p<0.1; **p<0.05; ***p<0.01}"),
          align=TRUE, omit.stat=c("aic", "theta" ),
          out="Table A4 (AllEdits(Table_A4_Appendix)_ZinB_Specifications_Zero).html",  type = "text")
...
# as data was just avaiable from 2009 upwards and excluding sessions 109-111 would reduce the data set so drastically in size that the zinb-models do not work anymore 
